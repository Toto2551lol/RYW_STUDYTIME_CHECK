...
export default function StudentDashboard() {
  ...
  // ===== AUTO LOAD PROFILE TIMETABLE (my timetable -> class timetable) =====
  useEffect(() => {
    ...
    const loadProfileTimetable = async () => {
      try {
        ...
        // 2) ถ้ายังไม่มี timetable ส่วนตัว → ดึงของห้อง
        if (!hasPersonal) {
              ...
              <table className="min-w-full text-[11px] sm:text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-100">
                    <th className="px-2 py-2 text-left">รหัสวิชา</th>
                    <th className="px-2 py-2 text-left">ชื่อวิชา</th>
                    <th className="px-2 py-2 text-center">หน่วยกิต</th>
                    ...
                    <th className="px-2 py-2 text-center">% ขาด</th>
      ...
      {activeTab === "profile" && (
        <section className="rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/85 p-4 sm:p-5 shadow">
          ...
          >
            ...
            <div className="max-h-72 sm:max-h-80 overflow-auto rounded-2xl border border-slate-200 dark:border-slate-700 -mx-2 sm:mx-0">
                      ...
                      {PERIODS.map((p) => (
                        <td key={p} className="px-1 py-1">
                          <select
                            ...
                            className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white text-slate-900 dark:bg-slate-900 dark:text-slate-100 px-1 py-0.5 text-[10px] sm:text-xs focus:outline-none focus:ring-1 focus:ring-slate-500/70"
                          >
